...
import { fechaLocal, fechaTexto, fechaTurno, horaCorta, tituloDia } from '@/lib/fechas';
import type { Perfil, Sucursal, TestDrive } from '@/lib/tipos';

const ESTADO: Record<TestDrive['estado'], { clase: string; texto: string }> = {
...
export function AgendaTestDrive({ turnos, sucursales, modelos, leads, yo }: {
  ...
  sucursales: Sucursal[];
  modelos: string[];
  leads: { id: number; nombre: string; telefono: string }[];
  yo: Perfil;
}) {
  ...
  const delDia = activos.filter((t) => t.fecha === dia);
  const misSolicitudes = turnos.filter((t) => t.vendedor_id === yo.id && t.fecha >= hoy);
  ...
  return (
    <div className="dash">
      <div className="td-grid">
        <div className="dcard">
          <Calendario mes={mes} seleccionado={dia} conEventos={new Set(activos.map((t) => t.fecha))} onSeleccionar={setDia} onCambiarMes={setMes} />
        </div>
          ...
          <div className="dcard">
            ...
            {delDia.length ? delDia.map((t) => (
              <div key={t.id} className="slot-row">
                <div className="slot-time">{horaCorta(t.hora)}</div>
                <div style={{ flex: 1 }}><div className="slot-veh">{t.vehiculo}</div><div className="slot-cli">{t.cliente}</div></div>
      ...
      <div className="dcard" style={{ marginTop: 18 }}>
        ...
        {misSolicitudes.length ? misSolicitudes.map((t) => (
          <div key={t.id} className="td-req-row">
            ...
            <div className="td-req-info"><div className="td-req-veh">{t.vehiculo} · {t.cliente}</div><div className="td-req-sub">{horaCorta(t.hora)} · Sucursal {sucursales.find((s) => s.id === t.sucursal_id)?.nombre ?? '—'}</div></div>
            <span className={`status-pill ${ESTADO[t.estado].clase}`}>{ESTADO[t.estado].texto}</span>
          </div>
